...
export class RoomImp implements Room {
  ...
  private mTriggerPopup: () => void;
  private mTriggerInputFocus: () => void;
  ...
  public addMessageSentFromAnotherSession(message: MessageSent) {
    this.getPluginManager().triggerPlugins(RoomImp.MessageSentFromAnotherSessionPlugin, message);
    this.setLastActivity(message);
  ...
  public addMessageReceived(message: MessageReceived): void {
    this.getPluginManager().triggerPlugins(RoomImp.MessageReceivedPlugin, message);
    this.setLastActivity(message);
  ...
  public sendMessage(message: string, messageType: MessageType = MessageType.CHAT): void {
    ...
    this.getPluginManager().triggerPlugins(RoomImp.MessageSentPlugin, msgObj);
  ...
  public addWritingStatusEvent(writingStatus: MessageWritingStatus): void {
    this.getPluginManager().triggerPlugins(RoomImp.WritingStatusPlugin, writingStatus);
  ...
  public onTriggeredPopup(callback: () => void): void {
      this.mTriggerPopup = callback;
  }

  public triggerPopup(): void {
    if (typeof this.mTriggerPopup !== "undefined" && this.mTriggerPopup !== null) {
      this.mTriggerPopup();
    }
  }

  public onTriggeredInputFocus(callback: () => void): void {
      this.mTriggerInputFocus = callback;
  }

  public triggerInputFocus(): void {
    if (typeof this.mTriggerInputFocus !== "undefined" && this.mTriggerInputFocus !== null) {
      this.mTriggerInputFocus();
    }
  }
