...
function url2tag(fragment, options) {
    ...
    if (options.attributes === undefined)
        { options.attributes = []; }
    var optionsStr = options.attributes.map(function (attribute) {
        if (typeof attribute === 'function') {
            var attributeObj = attribute(fragment);
            var name = (attributeObj || {}).name;
  ...
  return "<a href=\"" + fragment.protocol + fragment.encoded + "\" " + optionsStr + ">" + original + "</a>"
}
});
